...
export default function CustomAutocomplete({ name, label, options, onSelect }: Props) {
  ...
  const {
    ...
    onChange: (event, newValue) => {
      setValue(newValue);
      if (onSelect) {
        onSelect({ [name]: newValue?.label || null });
      }
    },
      ...
      {groupedOptions.length > 0 && (
        <Listbox {...getListboxProps()} sx={{ zIndex: 999 }}>
          {(groupedOptions as typeof options).map((option, index) => {
            ...
            return <Option key={key} {...restProps}>{option.label}</Option>; // Passando a key diretamente
          })}
        </Listbox>
      )}
    </div>
